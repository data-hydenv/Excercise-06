...
```{sql connection=con}
select * from day_ind
JOIN night_ind ON day_ind.hobo_id=night_ind.hobo_id
```

```{sql connection=con}
select * 
from day_ind
INNER JOIN night_ind
ON day_ind.hobo_id = night_ind.hobo_id
LEFT JOIN hobos
ON hobos.id = night_ind.hobo_id
...
SELECT *
FROM 
    (select hobo_id, avg(temperature) as mean_night,
    variance(temperature) as var_night,
    percentile_cont(0.9) within 
    group (order by temperature) as perc_90_night
    from raw_data 
    where date_part('hour', tstamp) < 6 or date_part('hour', tstamp) >= 18
    group by hobo_id) night
LEFT JOIN
    (select hobo_id, avg(temperature) as mean_day, 
    variance(temperature) as var_day,
    percentile_cont(0.9) within 
    group (order by temperature) as perc_90_day
    from raw_data 
    where date_part('hour', tstamp) >= 6 and date_part('hour', tstamp) < 18
    group by hobo_id) day
ON day.hobo_id = night.hobo_id
LEFT JOIN hobos
ON hobos.id = night.hobo_id
```
